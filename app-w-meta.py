...
def generate_image(text, img1, img2, img3, height, width, guidance_scale, img_guidance_scale, inference_steps, seed, separate_cfg_infer, offload_model,
    ...
    # clear the output image, to avoid lack of detected changes
    img = None

    input_images = [img1, img2, img3]
    # Delete None
    input_images = [imgI for imgI in input_images if imgI is not None]
    ...
    if save_images:
        ...
        meta.add_text("Model", "OmniGen")
        meta.add_text("Prompt", text)
        meta.add_text("Height", str(height))
        meta.add_text("Width", str(width))
        meta.add_text("Guidance Scale", str(guidance_scale))
        meta.add_text("Image Guidance Scale", str(img_guidance_scale))
        meta.add_text("Inference Steps", str(inference_steps))
        meta.add_text("Seed", str(seed))
        meta.add_text("Separate CFG Infer", str(separate_cfg_infer))
        meta.add_text("Offload Model", str(offload_model))
        meta.add_text("Use Input Image Size As Output", str(use_input_image_size_as_output))
        meta.add_text("Max Input Image Size", str(max_input_image_size))
        meta.add_text("Run Time", str(run_time))
    ...
    with gr.Row():
        with gr.Column():
            ...
            separate_cfg_infer = gr.Checkbox(
                label="separate_cfg_infer", info="Whether to use separate inference process for different guidance. This will reduce the memory cost.", value=False,
            )
            offload_model = gr.Checkbox(
                label="offload_model", info="Offload model to CPU, which will significantly reduce the memory cost but slow down the generation speed. You can cancel separate_cfg_infer and set offload_model=True. If both separate_cfg_infer and offload_model are True, further reduce the memory, but slowest generation", value=True,
            )
